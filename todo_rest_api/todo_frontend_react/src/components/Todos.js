...
const Todos = () => {
  ...
  const getTodos = useCallback(async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_HOST_URL}/todos`);
  ...
  const deleteTodoHandler = async (todoId) => {
    const response = await fetch(`${process.env.REACT_APP_HOST_URL}/todos/` + todoId, {
      method: 'DELETE',
    });
  ...
  const submitHandler = async (event) => {
    ...
    setEditedTodo(null);
    setEnteredText('');
    let url = `${process.env.REACT_APP_HOST_URL}/todos`;
    let method = 'POST';
    if (editedTodo) {
